...
이번 [자율 프로젝트](https://github.com/Mockin-2024)를 진행하며 API 문서 관리 방식에 근본적인 문제를 느꼈습니다.

처음에는 Postman을 활용해 문서를 관리했지만, 실제 구현과 문서가 자주 어긋나면서 팀원들이 “이 API, 파라미터 바뀌었나요?” 같은 질문을 반복하고 변경 내용을 구두나 메신저로 수시로 전달하는 일이 많아졌어요.   
결국, 테스트 단계에서는 문서와 실제 구현의 불일치 때문에 불필요한 혼선과 시간 낭비가 이어졌습니다.

이런 비효율을 자동화·일원화하지 않으면 개발 효율도, 서비스 품질도 한계가 명확하다는 판단이 들었습니다.  
그래서 **“API 문서를 자동으로 최신화하고, 테스트를 통과한 것만 신뢰할 수 있게 만들자”**라는 목표로 문서 자동화 도구를 본격적으로 검토하게 됐습니다.
...
단순한 커뮤니케이션 비용이 누적되며, 팀 전체의 생산성과 집중도에 생각보다 큰 영향을 주고 있었습니다.

### 1-2. 기술 선택을 위한 비교

API 문서 자동화 도구로 Swagger와 Spring REST Docs 두 가지를 비교했습니다.
